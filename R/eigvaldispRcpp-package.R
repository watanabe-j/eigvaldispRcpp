...
#'
#' This package extends the main package \pkg{eigvaldisp} with
#' fast \proglang{C++} functions via the \pkg{Rcpp} interface
#'
...
#'   [https://github.com/watanabe-j/eigvaldisp](https://github.com/watanabe-j/eigvaldisp).
#'
...
#' with Pan and Frank's approximation.
#'
#' These are internal \proglang{C++} functions to be used in within
#' \code{eigvaldisp:::AVar.VRR_pfc()} (which in turn is typically called by
#' the front-end \code{eigvaldisp::Var.VRR()}).  The choice among these
#' \proglang{C++} functions is made by the argument \code{cppfun} in
#' the outside \R functions.
#'
...
#'
#' \code{Cov_r2A} and \code{Cov_r2E} are parallelized with \proglang{OpenMP},
#' whereas \code{Cov_r2P} is with \proglang{IntelTBB} via \code{RcppParallel}
#' (when the environment allows).  They tend to show very similar performance,
#' though substantial environment-specific variation has been
#' noted.  \code{Cov_r2P} seems to show similar performance across various
#' environments, but tends to be slightly slower than the others when
#' \proglang{OpenMP} is properly supported.  It also does not allow the user
#' to specify the number of threads, hence typically ends up using all
#' processors available.  On the other hand, the number of threads can be
#' specified in \code{Cov_r2A} and \code{Cov_r2E}
#' thanks to \proglang{OpenMP}.  Unfortunately, however, \proglang{OpenMP}
#' does not seem to have a native support in mac environments
...
#' controlled by the argument \code{nthreads} in the front-end \R
#' functions.  The default value \code{0L} automatically sets itself into
#' one-half of the number of processors detected, i.e.,
#' \code{omp_get_num_procs()/2}.  Setting this value beyond the number of
#' physical cores typically does not
...
#' ncol = length(n))}.  Internally, \code{n}, \code{R} and \code{E} are
#' converted into \code{Rcpp::NumericVector} in \code{Cov_r2C()} and
#' \code{Cov_r2P()}, \code{arma::vec} in \code{Cov_r2A()},
...
#'
#' @param n
#'   Degrees of freedom (not sample sizes); numeric of length \code{1} or more
#' @param R
...
#'   detected.  See \dQuote{Details}.
#'
#' @return
#'   A numeric vector of
#'   \eqn{\sum 2 \mathrm{Cov}(r_{ij}^2, r_{kl}^2)}{\sum 2 Cov(r_{ij}^2, r_{kl}^2)},
